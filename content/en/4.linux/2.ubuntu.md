...
📌 **TL;DR**: A complete guide to secure an Ubuntu server right after provisioning: system configuration, user management, SSH hardening and security best practices.
::
...
📦 **Recommended versions**: Ubuntu 22.04 LTS or 24.04 LTS for long-term support and the latest security features.
::
...
⚠️ **Warning**: A reboot (`reboot`) can temporarily interrupt services. Plan this during a maintenance window.
::
...
::callout{icon="i-heroicons-information-circle"}
💡 **Difference**: `adduser` is interactive and user-friendly; `useradd` is more direct and scriptable.
::
...
::callout{icon="i-heroicons-exclamation-triangle" color="amber"}
⚠️ **Important**: Changing the SSH port is **not sufficient** protection by itself. It's an additional defense layer, not a replacement for strong authentication (SSH keys) and a firewall.
::
...
::callout{icon="i-heroicons-light-bulb" color="blue"}
💡 **Port choice** : Ports 49152-65535 are dynamic/private ports rarely used by other services. Avoid common choices like 2222 or 2022.
::
...
- ✅ **Use 2FA for SSH**: With Google Authenticator or Duo
  ::
...
- ❌ **Never close your SSH session** before verifying the new port in a second session
- ❌ **Don't forget to configure the firewall** for the new SSH port
- ❌ **Never disable password authentication** before testing key-based logins
- ❌ **Do not use reserved ports** (< 1024) without appropriate root privileges
- ⚠️ **Beware of overly restrictive firewall rules** that could lock you out
- ⚠️ **Document your configuration**: note the custom SSH port in a secure password manager
  ::
