...
<template>
  ...
  >
    <template #body>
      <ContentRenderer v-if="docContent?.body" :value="docContent" class="markdown-content" />
      ...
      <div v-else class="text-center py-12">
        ...
        <p class="text-sm text-muted mb-4">
          Le document <code class="px-2 py-1 bg-elevated rounded">{{ id }}</code> n'existe pas ou
          est vide.
        </p>
      </div>
    </template>
    ...
    <template #footer>
      <div class="flex justify-end gap-2">
        <UButton label="Fermer" color="neutral" variant="outline" @click="emit('close')" />
      </div>
    </template>
  </UModal>
</template>
